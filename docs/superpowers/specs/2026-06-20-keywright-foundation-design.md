...
1. **A user-facing how-to** (`HOWTO.txt`, or folded into `summary.txt`) covering **key-lifecycle operations the keyholder may need — in particular changing an existing UID (name/email) or adding a new one.** Because UIDs are self-certified by the **offline primary (certify) key**, which is **never on the YubiKey** (the card holds only the sign/encrypt/auth subkeys), the keyholder **cannot** do this with their card alone. The how-to instructs them to **contact the security team**, who restore the certify key from the LUKS escrow, update the UID, and **re-issue + re-publish the public certificate**. Reassurance: the keyholder's **YubiKey, PINs, and subkeys are unaffected** — no new card, no re-`keytocard`; only the public cert (and any keyservers) change. \*(Underlying security-team mechanics, for the Plan-3 operator runbook: restore the certify key from a LUKS backup → `gpg --edit-key <fpr>` → `adduid` (add) / `revuid` (retire) / `uid N` + `primary` (set primary) → `save` → re-export `<keyid>-<date>.asc` and re-push. UID change is one instance of the broader certify-key lifecycle — subkey renewal/rotation, revocation — all security-team-mediated; see §17.)
